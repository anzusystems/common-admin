...
const focusField = async (wrapper: Wrapper) => {
  await userEvent.click(wrapper.find('.v-field').element as HTMLElement)
  await flush()
  return inputOf(wrapper)
}
...
const backspaceOverTheField = async (wrapper: Wrapper) => {
  const input = await focusField(wrapper)
