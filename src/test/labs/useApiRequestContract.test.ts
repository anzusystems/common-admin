...
describe('what useApiRequest throws', () => {
  it('tells a forbidden response from the rest', async () => {
    const { executeRequest } = buildApi(failingWith(axiosError({ response: { status: 403 } })))

    await expect(executeRequest()).rejects.toBeInstanceOf(AnzuApiForbiddenError)
  })
  ...
  it('tells a validation failure from the rest', async () => {
    ...
    const { executeRequest } = buildApi(failingWith(error))

    await expect(executeRequest()).rejects.toBeInstanceOf(AnzuApiValidationError)
  })
  ...
  it('tells a dependency conflict from a validation failure', async () => {
    ...
    await expect(executeRequest()).rejects.toBeInstanceOf(AnzuApiDependencyExistsError)
  })
  ...
  it('tells a forbidden operation from a validation failure', async () => {
    ...
    await expect(executeRequest()).rejects.toBeInstanceOf(AnzuApiForbiddenOperationError)
  })

  it('tells a timeout from a backend that answered', async () => {
    const { executeRequest } = buildApi(failingWith(axiosError({ code: 'ECONNABORTED' })))

    await expect(executeRequest()).rejects.toBeInstanceOf(AnzuApiTimeoutError)
  })
  ...
  it('hands back every other axios failure with the response on its cause', async () => {
    ...
    await expect(executeRequest()).rejects.toBeInstanceOf(AnzuApiAxiosError)
    await executeRequest().catch((thrown: AnzuApiAxiosError) => {
      expect(thrown.cause.response?.status).toBe(401)
    })
  })

  it('keeps an unexpected status code as its own kind', async () => {
    const { executeRequest } = buildApi(vi.fn().mockResolvedValue({ status: 418, data: { teapot: true } }))

    await expect(executeRequest()).rejects.toBeInstanceOf(AnzuApiResponseCodeError)
  })

  it('falls back to fatal for something that did not come from axios', async () => {
    const { executeRequest } = buildApi(failingWith(new Error('a bug in a callback')))

    await expect(executeRequest()).rejects.toBeInstanceOf(AnzuFatalError)
  })
})

describe('what useApiRequest answers with', () => {
  it('returns the body when there is one', async () => {
    const { executeRequest } = buildApi(vi.fn().mockResolvedValue({ status: 200, data: { id: 7 } }))

    await expect(executeRequest()).resolves.toEqual({ id: 7 })
  })
  ...
  it('answers undefined for a no-content success', async () => {
    ...
    const { executeRequest } = buildApi(vi.fn().mockResolvedValue({ status: 204, data: undefined }))

    await expect(executeRequest()).resolves.toBeUndefined()
  })

  it('answers undefined for an accepted request as well', async () => {
    const { executeRequest } = buildApi(vi.fn().mockResolvedValue({ status: 202, data: undefined }))

    await expect(executeRequest()).resolves.toBeUndefined()
  })

  it('treats a valid status with no body as a failure', async () => {
    const { executeRequest } = buildApi(vi.fn().mockResolvedValue({ status: 200, data: undefined }))
...
describe('what useApiRequest sends', () => {
  it('serialises the body, the way the helper it replaces did', async () => {
    ...
    await executeRequest({ object: { title: 'a' } })
  ...
  it('lets a call override the url it was built with', async () => {
    ...
    await executeRequest({ urlTemplate: '/other/:id', urlParams: { id: 3 } })
  ...
  it('keeps the console to itself unless asked otherwise', async () => {
    ...
    await expect(silent.executeRequest()).rejects.toBeInstanceOf(AnzuApiAxiosError)
    expect(consoleError).not.toHaveBeenCalled()

    const loud = buildApi(failingWith(axiosError({ response: { status: 500 } })), false)
    await expect(loud.executeRequest()).rejects.toBeInstanceOf(AnzuApiAxiosError)
...
describe('what useApiRequest does with an abort', () => {
  it('rejects, because the branch meant to catch an abort never sees one', async () => {
    ...
    const { executeRequest } = buildApi(failingWith(canceled))
