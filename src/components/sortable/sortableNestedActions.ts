...
export const SortableLevel: InjectionKey<number> =
  Symbol.for("anzu:SortableLevel");

export interface SortableNestedItem<
  TData extends SortableItemWithParentDataAware = any,
> {
  data: TData;
...
export interface SortableNested<
  TData extends SortableItemWithParentDataAware = any,
> {
...
export function useSortableNestedActions(
  ...
  const forceRerenderWidgetHtml = (
    emitData: SortableNested | undefined = undefined,
  ) => {
    forceRerender.value++;
  ...
  const moveArrayElement = (
    ...
    const { itemFound: itemToMove, parentItem: itemToMoveParentItem } =
      findItemById(currentId, clonedData.children);
    ...
    const { itemFound: targetItem } = findItemById(
      targetId,
      clonedData.children,
    );
  ...
  const addAfterId = (
    ...
    const { itemFound: targetItem, parentItem: targetItemParentItem } =
      findItemById(targetId, clonedData.children);
