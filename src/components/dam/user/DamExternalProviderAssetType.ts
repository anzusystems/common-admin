...
export function useDamExternalProviderAssetType() {
  ...
  ).map(([providerName, value]): ValueObjectOption<string> => {
    return {
      value: providerName,
      title: value.title,
    };
  ...
  const allExternalProviderAssetTypeOptions =
    ref<ValueObjectOption<string>[]>(all);

  const getExternalProviderAssetTypeOption = (value: string) => {
    return allExternalProviderAssetTypeOptions.value.find(
