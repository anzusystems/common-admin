...
const commonAdminCoreDamOptions = ref<CommonAdminCoreDamOptions | undefined>(
  undefined,
);
...
export function useCommonAdminCoreDamOptions(configName: string = "default") {
  ...
  return {
    ...
    endPointImage:
      commonAdminCoreDamOptions.value.configs[configName].endPointImage ||
      "/adm/v1/image",
    endPointAsset:
      commonAdminCoreDamOptions.value.configs[configName].endPointAsset ||
      "/adm/v1/asset",
    endPointRoi:
      commonAdminCoreDamOptions.value.configs[configName].endPointRoi ||
      "/adm/v1/roi",
    mainFileSingleUseEnabled:
      commonAdminCoreDamOptions.value.configs[configName]
        .mainFileSingleUseEnabled ?? true,
    showSourceEnabled:
      commonAdminCoreDamOptions.value.configs[configName].showSourceEnabled ??
      true,
    showFileInfoEnabled:
      commonAdminCoreDamOptions.value.configs[configName].showFileInfoEnabled ??
      true,
    sourceLabel:
    ...
    assetListEnabledFilters:
      commonAdminCoreDamOptions.value.configs[configName]
        .assetListEnabledFilters, // defaults to undefined = all filters
    simpleAssetSidebarEnabled:
      commonAdminCoreDamOptions.value.configs[configName].simpleAssetSidebar ??
      false,
  };
}
