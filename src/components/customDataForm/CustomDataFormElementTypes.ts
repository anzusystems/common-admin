...
export const CustomDataFormElementTypeDefault =
  CustomDataFormElementType.String;
...
export function useCustomDataFormElementType() {
  const { t } = useI18n();
  const customDataFormElementTypeOptions = ref<
  ...
  const getCustomDataFormElementTypeOption = (
    value: CustomDataFormElementTypeType,
  ) => {
    return customDataFormElementTypeOptions.value.find(
