...
export function useFormatAndSizeCheck(
  ...
  const checkFormatsAndSizes = (files: File[], disableAlert = false) => {
    ...
    const validFiles = files.filter((file) => {
      const isFileValid =
        checkFormats(file, acceptKeys.value) &&
        checkSizes(file, maxSizeKeys.value, maxSizes);
      if (!isFileValid) {
    ...
    if (incorrectFileNames.length && !disableAlert) {
      ...
      const { t } = i18n.global;
      showWarning(
        t("common.system.upload.incorrectFormatSize") +
          ":" +
          incorrectFileNames.join(", "),
      );
    }
