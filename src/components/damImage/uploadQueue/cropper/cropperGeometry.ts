...
export const isUsableAspectRatio = (aspectRatio: number): boolean =>
 ...
 */
export const initialSelectionRect = (
  bounds: Size,
  aspectRatio: number,
  coverage: number,
): Rect => {
...
const availableAlong = (
  ...
  if (side === "start")
    return Math.max(0, boundsEnd - Math.max(start, boundsStart));
  if (side === "end")
    return Math.max(0, Math.min(start + length, boundsEnd) - boundsStart);
...
export const clampRectInside = (
  ...
  const maxHeight = availableAlong(
    asked.y,
    asked.height,
    bounds.y,
    bounds.height,
  ...
  if (isUsableAspectRatio(aspectRatio)) {
    ...
    const scale = Math.min(
      1,
      width > 0 ? maxWidth / width : 1,
      height > 0 ? maxHeight / height : 1,
    );
    width *= scale;
    height *= scale;
  } else {
