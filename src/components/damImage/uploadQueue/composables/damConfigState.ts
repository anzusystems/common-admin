...
export function useDamConfigState(
  client: undefined | (() => AxiosInstance) = undefined,
) {
      ...
      fetchPubConfiguration(client)
        .then((config) => {
          if (Object.keys(config).length < 1) {
            throw new Error(
              "Unable to load pub config. Incorrect response body.",
            );
          }
  ...
  function setDamPubConfig(data: DamPubConfig) {
    try {
      ...
      throw new Error(
        "Unable to load dam pub config. Incorrect fields in json.",
      );
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      fetchConfiguration(client)
        .then((config) => {
          if (Object.keys(config).length < 1) {
            throw new Error(
              "Unable to load dam prv config. Incorrect response body.",
            );
          }
  ...
  function setDamPrvConfig(data: DamPrvConfig) {
    try {
      ...
      damConfigStore.damPrvConfig.assetExternalProviders =
        data.assetExternalProviders;
    ...
    return new Promise((resolve, reject) => {
      ...
      fetchExtSystemConfiguration(extSystemId, client)
        .then((config) => {
          if (Object.keys(config).length < 1) {
            throw new Error(
              "Unable to load dam ext system config. Incorrect response body.",
            );
          }
  ...
  function setDamConfigExtSystem(
    data: DamExtSystemConfig,
    ...
    try {
      ...
      throw new Error(
        "Unable to load dam ext system config. Incorrect fields in json.",
      );
    }
  }
  ...
  function loadDamConfigAssetCustomFormElements(
    ...
    return new Promise((resolve, reject) => {
      ...
      const promises = types.map((type) =>
        fetchAssetCustomFormElements(client, extSystemId, type),
      );
      ...
      Promise.allSettled(promises)
        .then((results) => {
          ...
          if (loadedTypes.length === 0 && types.length > 0) {
            throw new Error(
              "Unable to load asset custom form config. Incorrect response body.",
            );
          }
          setDamConfigAssetCustomFormElements(
            loadedResponses,
            extSystemId,
            loadedTypes,
          );
          if (failedTypes.length > 0) {
            onConfigError(
              new Error(
                `Unable to load asset custom form config for: ${failedTypes.join(", ")}.`,
              ),
            );
          }
          resolve(true);
  ...
  function setDamConfigAssetCustomFormElements(
    ...
    try {
      const existingConfig =
        damConfigStore.damConfigAssetCustomFormElements.get(extSystemId) || {
          image: [],
          audio: [],
          video: [],
          document: [],
      ...
      throw new Error(
        "Unable to load asset custom form config. Incorrect fields in json.",
      );
    }
  }
  ...
  function loadDamConfigDistributionCustomFormElements(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (
        damConfigStore.damConfigDistributionCustomFormElements.has(
  ...
  async function getOrLoadDamConfigExtSystemByLicences(
    ...
    return responses
      .filter(
        (
          result,
        ): result is PromiseFulfilledResult<DamConfigLicenceExtSystemReturnType> =>
  ...
  async function getOrLoadDamConfigExtSystemByLicence(
    ...
    let foundLicenceConfig =
      damConfigStore.damConfigLicenceExtSystem.get(licence);
    ...
    if (isUndefined(foundLicenceConfig)) {
      try {
        ...
        damConfigStore.damConfigLicenceExtSystem.set(
          licence,
          foundLicenceConfig,
        );
      } catch (error) {
    ...
    let foundExtSystemConfig = damConfigStore.damConfigExtSystem.get(
      foundLicenceConfig.extSystem,
    );

    if (isUndefined(foundExtSystemConfig)) {
      try {
        await loadDamConfigExtSystem(foundLicenceConfig.extSystem);
        foundExtSystemConfig = damConfigStore.damConfigExtSystem.get(
          foundLicenceConfig.extSystem,
        );
      } catch (error) {
        console.error(
          `Error loading extension system ${foundLicenceConfig.extSystem}:`,
          error,
        );
        return undefined;
      }
  ...
  async function getExtSystemByLicence(
    licence: IntegerId,
  ): Promise<IntegerId | null> {
