...
const finishUpload = async (queueItem: UploadQueueItem, sha: string) => {
  const { damClient, endPointImage, endPointAsset } =
    useCommonAdminCoreDamOptions();
  ...
  for (const [key, values] of Object.entries(data.fields)) {
    switch (key) {
      ...
      default:
        // @ts-ignore
        errorMessages.push(
          t("common.damImage.uploadErrors.systemError") +
            ": " +
            key +
            " - " +
            values.join(","),
...
export function useUpload(
  ...
  function progressCallback(progressEvent) {
    ...
    const dataSent =
      lastLoaded > 0 ? progressEvent.loaded - lastLoaded : progressEvent.loaded;
    lastLoaded =
      progressEvent.total === progressEvent.loaded ? 0 : progressEvent.loaded;
      ...
      try {
        ...
        if (
          ...
          queueItem.error.message =
            "Upload chunk validation failed. Please contact administrator.";
    ...
    function speedCheckRun() {
      ...
      if (speedStack.length > 0) {
        const avgSpeed = Math.ceil(
          speedStack.reduce((sum, current) => sum + current) /
            speedStack.length,
        );
        const remainingBytes = Math.ceil(
          fileSize.value * ((100 - progress.value) / 100),
        );
