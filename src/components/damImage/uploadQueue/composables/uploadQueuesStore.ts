...
import {
  uploadStop,
  useUpload,
} from "@/components/damImage/uploadQueue/composables/uploadService";
...
export const useUploadQueuesStore = defineStore(
  "commonUploadQueuesStore",
  () => {
    ...
    async function addByCopyToLicence(
      ...
      if (isUndefined(configExtSystem)) {
        throw new Error(
          "useUploadQueuesStore.addByCopyToLicence: Ext system must be initialised.",
        );
      }
    ...
    async function addByFiles(
      ...
      if (isUndefined(configExtSystem)) {
        throw new Error(
          "useUploadQueuesStore.addByFiles: Ext system must be initialised.",
        );
      }
      for await (const file of files) {
        const type = getAssetTypeByMimeType(
          damFileTypeFix(file),
          configExtSystem,
        );
    ...
    function processUpload(queueKey: UploadQueueKey) {
      const waitingItems = getQueueItemsByStatus(
        queueKey,
        UploadQueueItemStatus.Waiting,
      );
      ...
      const uploadingCount = getQueueItemsByStatus(
        queueKey,
    ...
    async function queueItemUploadStart(
        ...
        queues.value.forEach((queue, queueKey) => {
          queue.items.forEach((item) => {
            if (item.assetId === asset.id && asset.mainFile && item.type) {
              ...
              item.mainFileInternal = asset.mainFileInternal;
              updateNewNames(
                asset.metadata.authorSuggestions,
                queue.suggestions.newAuthorNames,
              );
              updateNewNames(
                asset.metadata.keywordSuggestions,
                queue.suggestions.newKeywordNames,
              );
              item.authorConflicts = getAuthorConflicts(
                asset.metadata.authorSuggestions,
              );
    ...
    async function queueItemDuplicate(
      ...
      const { updateNewNames, getAuthorConflicts } = useAssetSuggestions();
      if (!originAssetFile || !assetType || assetType !== DamAssetType.Image)
        return;
      let assetRes: null | AssetDetailItemDto = null;
      try {
        assetRes = await fetchAssetByFileId(
          damClient,
          endPointAsset,
          originAssetFile,
        );
      ...
      queues.value.forEach((queue, queueKey) => {
        queue.items.forEach((item) => {
          ...
          if (item.assetId === assetId) {
            ...
            updateNewNames(
              assetRes.metadata.authorSuggestions,
              queue.suggestions.newAuthorNames,
            );
            updateNewNames(
              assetRes.metadata.keywordSuggestions,
              queue.suggestions.newKeywordNames,
            );
            item.authorConflicts = getAuthorConflicts(
              assetRes!.metadata.authorSuggestions,
    ...
    async function queueItemFailed(
      assetId: DocId,
    ...
    function stopUpload(queueKey: UploadQueueKey) {
      const queue = queues.value.get(queueKey);
      if (!queue || queue.items.length === 0) return;
      const currentItems = getQueueItemsByStatus(
        queueKey,
        UploadQueueItemStatus.Uploading,
      );
      queue.items.forEach((item) => {
