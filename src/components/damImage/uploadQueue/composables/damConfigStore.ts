...
export const useDamConfigStore = defineStore(
  "commonAdminDamConfigStore",
  () => {
    ...
    const damConfigExtSystem = shallowRef(
      new Map<IntegerId, DamExtSystemConfig>(),
    );
    const damConfigLicenceExtSystem = shallowRef(
      new Map<IntegerId, DamConfigLicenceExtSystem>(),
    );
    const damConfigAssetCustomFormElements = shallowRef(
      new Map<
        IntegerId,
        { [key in DamAssetTypeType]: CustomDataFormElement[] }
      >(),
    );
