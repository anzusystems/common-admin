...
export function useCollabAnyDataChange(
  room: CollabRoom,
  ...
  const anyChangeCallback = ref<
    | undefined
    | ((field: CollabFieldName, payload: CollabFieldDataEnvelope) => void)
  >();
  ...
  const anyChangeEventBusListener = (
    ...
    if (
      event.room !== room ||
      isUndefined(payload) ||
      isUndefined(anyChangeCallback.value)
    ) {
      return;
    }
  ...
  const addCollabAnyDataChangeListener = (
    ...
    unsubscribeCollabAnyDataChangeListener.value = changeEventBus.on(
      anyChangeEventBusListener,
    );
  };
