...
export function useCollabRoom(
  ...
  addToCachedUsers:
    ((...args: AddToCachedArgs<IntegerId>) => void) | undefined = undefined,
  ...
  const requestToTakeModerationEventBus =
    useCollabRequestToTakeModerationEventBus();
  ...
  const approvedRequestToTakeModerationEventBus =
    useCollabApprovedRequestToTakeModerationEventBus();
  const unsubscribeApprovedRequestToTakeModerationListener = ref<
    undefined | Fn
  >();
  ...
  const rejectedRequestToTakeModerationEventBus =
    useCollabRejectedRequestToTakeModerationEventBus();
  const unsubscribeRejectedRequestToTakeModerationListener = ref<
    undefined | Fn
  >();
  ...
  const approvedJoinRequestEventBusListener = (
    event: CollabApprovedJoinRequestEvent,
  ) => {
  ...
  const rejectedJoinRequestEventBusListener = (
    event: CollabRejectedJoinRequestEvent,
  ) => {
  ...
  const requestToTakeModerationEventBusListener = (
    event: CollabRequestToTakeModerationEvent,
  ) => {
    if (
      event.room === room &&
      isDefined(requestToTakeModerationCallback.value)
    ) {
  ...
  const approvedRequestToTakeModerationEventBusListener = (
    ...
    if (
      event.room === room &&
      isDefined(approvedRequestToTakeModerationCallback.value)
    ) {
  ...
  const rejectedRequestToTakeModerationEventBusListener = (
    ...
    if (
      event.room === room &&
      isDefined(rejectedRequestToTakeModerationCallback.value)
    ) {
      rejectedRequestToTakeModerationCallback.value();
    }
  };

  const kickedFromCollabRookEventBusListener = (
    event: CollabKickedFromRoomEvent,
  ) => {
  ...
  const addJoinRequestListener = (
    ...
    unsubscribeJoinRequestListener.value = joinRequestEventBus.on(
      joinRequestEventBusListener,
    );
  };
  ...
  const addApprovedJoinRequestListener = (callback: () => void) => {
    ...
    unsubscribeApprovedJoinRequestListener.value =
      approvedJoinRequestEventBus.on(approvedJoinRequestEventBusListener);
  };
  ...
  const addRejectedJoinRequestListener = (callback: () => void) => {
    ...
    unsubscribeRejectedJoinRequestListener.value =
      rejectedJoinRequestEventBus.on(rejectedJoinRequestEventBusListener);
  };
  ...
  const addRequestToTakeModerationListener = (
    ...
    unsubscribeRequestToTakeModerationListener.value =
      requestToTakeModerationEventBus.on(
  ...
  const addApprovedRequestToTakeModerationListener = (callback: () => void) => {
    ...
    unsubscribeApprovedRequestToTakeModerationListener.value =
      approvedRequestToTakeModerationEventBus.on(
  ...
  const addRejectedRequestToTakeModerationListener = (callback: () => void) => {
    ...
    unsubscribeRejectedRequestToTakeModerationListener.value =
      rejectedRequestToTakeModerationEventBus.on(
  ...
  const addCollabReconnectListener = (callback: () => void) => {
    ...
    unsubscribeCollabReconnectListener.value = reconnectEventBus.on(
      reconnectEventBusListener,
    );
  };
  ...
  const joinCollabRoom = async (
    ...
    return new Promise((resolve, reject) => {
      ...
      function markRoomInactiveOnFailedClaim() {
        if (isNewestWrite())
          collabRoomInfoState.set(room, createDefaultCollabRoomInfo());
      }
    });
  };
        ...
        .emit(
          ...
          (error, response: CollabAccessRoomCallbackTypes) => {
            if (
              !error &&
              isNewestWrite() &&
              isCollabSuccessAccessRoomCallback(response)
            ) {
              collabRoomInfoState.set(room, response.room);
            }
            resolve();
  ...
  const requestToJoinCollabRoom =
    (): Promise<CollabRequestToJoinStatusType> => {
      return new Promise((resolve, reject) => {
        if (!collabOptions.value.enabled || isUndefined(collabSocket.value)) {
  ...
  const requestToTakeModeration =
    async (): Promise<CollabRequestToTakeModerationStatusType> => {
      return new Promise((resolve, reject) => {
        if (!collabOptions.value.enabled || isUndefined(collabSocket.value)) {
  ...
  const collabRoomLocks = computed(
    (): Map<CollabFieldName, CollabFieldLock> => {
