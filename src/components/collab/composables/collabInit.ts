...
export function useCollabInit() {
  ...
  const initCollab = () => {
    ...
    try {
      ...
      collabSocket.value.on(
        ...
        (room: CollabRoom, locks: CollabRoomLocks | null) => {
          if (!isNull(locks)) {
            ...
            if (
              !collabFieldLocksState.has(room) ||
      ...
      collabSocket.value.on(
        "collabRoomDataChanged",
        (room: CollabRoom, data: CollabRoomData) => {
      ...
      collabSocket.value?.on(
        "approvedRequestToTakeModeration",
        (room: CollabRoom) => {
          approvedRequestToTakeModerationEventBus.emit({ room });
        },
      );
      collabSocket.value?.on(
        "rejectedRequestToTakeModeration",
        (room: CollabRoom) => {
      ...
      collabSocket.value.on("disconnect", async (reason) => {
        collabRoomInfoState.forEach(
          (roomInfo: CollabRoomInfo) =>
            (roomInfo.status = CollabStatus.Inactive),
        );
        collabConnected.value = collabSocket.value?.connected ?? false;
